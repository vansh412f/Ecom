  ...
  return (
    <div className="flex items-center gap-6 border-b border-[var(--border)] py-6">
      ...
      <div className="flex items-center border border-[var(--border)] rounded-xl">
        <button onClick={() => updateQuantity(item.id, item.quantity - 1)} className="p-3 hover:bg-[var(--border)] rounded-l-xl"><Minus className="h-4 w-4" /></button>
        <input type="text" value={item.quantity} readOnly className="w-12 text-center bg-transparent" />
        <button onClick={() => updateQuantity(item.id, item.quantity + 1)} className="p-3 hover:bg-[var(--border)] rounded-r-xl"><Plus className="h-4 w-4" /></button>
      </div>
      <div>
        <p className="text-lg font-bold text-white">${(item.price * item.quantity).toLocaleString()}</p>
      </div>
      <button onClick={() => removeFromCart(item.id)} className="text-[var(--foreground-secondary)] hover:text-red-500">
        <X className="h-6 w-6" />
      </button>
    </div>
  );
